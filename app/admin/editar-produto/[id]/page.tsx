...
export default function EditarProdutoPage() {
    ...
    const checkAuthAndFetchCategories = async () => {
      ...
      if (id) {
        ...
        if (prodData) {
          ...
          if (prodData.colors && Array.isArray(prodData.colors)) {
            const mappedColors = prodData.colors.map((c: any) => ({
              name: c.name || "",
              hex: c.hex || "#000000",
              files: [],
              previews: [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const color of colorVariants) {
      let colorImages: string[] = color.existingImages ? [...color.existingImages] : []
      if (color.files && color.files.length > 0) {
        colorImages = [] // Substitui imagens antigas pelas novas
        for (const file of color.files) {
          ...
          colorImages.push(publicUrlData.publicUrl)
        }
      }
      finalColors.push({
        name: color.name,
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="border border-slate-200 rounded-xl p-6 bg-slate-50/50">
              <div className="flex items-center justify-between mb-4">
                ...
                <Button 
                  type="button" 
                  onClick={() => setColorVariants([...colorVariants, { name: "", hex: "#000000", files: [], previews: [], existingImages: [] }])}
                  variant="outline" 
              ...
              {colorVariants.length > 0 && (
                <div className="space-y-6">
                  {colorVariants.map((variant, vIndex) => {
                    const displayImages = variant.previews.length > 0 ? variant.previews : variant.existingImages
                    return (
                        ...
                        <div>
                          ...
                          <div className="border-2 border-dashed border-slate-200 rounded-lg p-4 hover:border-brand-plum transition-colors cursor-pointer bg-slate-50 flex flex-col items-center justify-center text-center relative">
                            <input
                              ...
                              multiple
                              accept="image/*"
                              required={variant.existingImages.length === 0 && variant.files.length === 0}
                              onChange={(e) => {
                                if (e.target.files) {
                                  const files = Array.from(e.target.files)
                                  const newVariants = [...colorVariants]
                          ...
                          {displayImages && displayImages.length > 0 && (
                            <div className="grid grid-cols-5 sm:grid-cols-6 gap-2 mt-3">
                              {displayImages.map((preview, pIndex) => (
